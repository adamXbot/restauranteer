<script lang="ts">
	...
	// Shared map + nearby-search surface, used by /near and the map-led top of
	// /discover. `from` tags the ?from= query on result links so the detail
	// page's Back returns to whichever page embedded the map.
	type MapPreferences = {
		default_map_provider: string;
		default_navigation_app: NavigationApp;
		attributes: { id: string; label: string }[];
	};
	// A rail-ready view of the current results, so an embedder (Discover) can
	// render its own compact row instead of the built-in results list.
	type NearRailItem = {
		key: string;
		name: string;
	...
	type NearState = { hasCenter: boolean; loading: boolean; items: NearRailItem[] };
	let {
		preferences,
		...
		allCuisines?: string[];
		from?: string;
		/** Hide the built-in results list (the embedder renders its own rail). */
		hideResults?: boolean;
		/** Fires with a rail-ready snapshot whenever the search changes. */
		onState?: (s: NearState) => void;
	} = $props();
	...
	const NEAR_SESSION_KEY = 'restauranteer.near.session';
	...
	// Rail snapshot for embedders: the pin's results, mapped to link-ready cards.
	const railItems = $derived.by<NearRailItem[]>(() => {
		if (!center) return [];
		return results.map((r) =>
			r.source === 'vault'
				? {
						...
						href: `/restaurant/${r.uuid}?from=${from}`
					}
				: {
						key: r.place_id,
						name: r.name,
						...
						href: `/place/${r.place_id}?from=${from}`
					}
		);
	});
	$effect(() => {
		onState?.({ hasCenter: !!center, loading, items: railItems });
	});
	const selectedNavHref = $derived(
		selectedPin
			? navigationUrl(
					...
					preferences.default_navigation_app
				)
			: null
	);
	const navLabel = $derived(labelForApp(preferences.default_navigation_app));
	...
	async function loadVaultPins() {
		...
		try {
			...
			const body = (await res.json()) as { pins: VaultPin[] };
			vaultPins = body.pins;
	...
	function openResult(r: Result) {
		if (r.source === 'vault') void goto(`/restaurant/${r.uuid}?from=${from}`);
		else void goto(`/place/${r.place_id}?from=${from}`);
	}

	function openVaultPin(pin: VaultPin) {
		void goto(`/restaurant/${pin.uuid}?from=${from}`);
	}
	...
	function openSelected() {
		...
		if (selectedPin.source === 'vault') void goto(`/restaurant/${selectedPin.uuid}?from=${from}`);
		else void goto(`/place/${selectedPin.place_id}?from=${from}`);
	}
	...
	async function runQuery() {
		...
		try {
			const params = new URLSearchParams({
				...
				source: googleEnabled ? 'all' : 'vault'
			});
	...
	type NearSession = {
		...
		ratingIdx: number;
		cuisines: string[];
		attributeFilter: Record<string, 'yes' | 'no'>;
		results: Result[];
	};

	// Persist the current search so returning to the map — via the on-screen
	// Back button or a browser back-swipe — keeps the dropped pin and results
	// instead of resetting to a blank map. Scoped to the tab session.
	function saveNearSession() {
		try {
			const payload: NearSession = {
				center,
				radiusIdx,
				ratingIdx,
				cuisines,
			...
			sessionStorage.setItem(NEAR_SESSION_KEY, JSON.stringify(payload));
		} catch {
			// sessionStorage may be blocked; the map still works, just no restore.
		}
	}

	function loadNearSession(): NearSession | null {
		try {
			const raw = sessionStorage.getItem(NEAR_SESSION_KEY);
			return raw ? (JSON.parse(raw) as NearSession) : null;
		} catch {
			return null;
		}
	}
	...
	onMount(() => {
		const saved = loadNearSession();
		if (saved) {
			center = saved.center;
			radiusIdx = saved.radiusIdx;
			ratingIdx = saved.ratingIdx;
			cuisines = saved.cuisines;
			attributeFilter = saved.attributeFilter;
	...
	$effect(() => {
		// Persist the search whenever any input or the results change.
		void center;
		void radiusIdx;
		void ratingIdx;
		void cuisines;
...
<div class="relative h-[40vh] w-full bg-panel">
...
<div class="flex flex-wrap items-center gap-2 px-5 pt-3">
	...
	{#each preferences.attributes as def (def.id)}
